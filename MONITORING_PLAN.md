...
- A dedicated Railway cron uses an optional Slack Incoming Webhook to post an untagged balance update every twelve hours, evaluate downward crossings at 4.0B/3.9B/3.8B/3.7B/3.6B/3.5B every ten minutes, send one full-recovery message, and report an untagged greater-than-10M balance drop between consecutive checks. The 3.8B through 3.6B crossings tag `@here` and request an internal SFF reaction; 3.5B is the critical `🚨 ACTION NEEDED` threshold. See [SLACK_SETUP.md](SLACK_SETUP.md).
