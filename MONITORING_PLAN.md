...
- `/api/history` and the dashboard chart expose 7-, 30-, 90-, and 180-day daily snapshots plus monthly snapshots from January 2025 onward. They are reconstructed from historical Ethereum state every six hours, so the RPC must support archive reads. The same page shows equivalent short ranges, SDE daily history from January 2026, and month-end points.
